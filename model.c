...
int allocate_terminal_id(ShmBuf *shmp) {
    int id;
    sem_wait(&shmp->sem);
    memcpy(&id, shmp->msgbuf, sizeof(int));
    id++;
    memcpy(shmp->msgbuf, &id, sizeof(int));
    sem_post(&shmp->sem);
    return id;
}
...
void model_send_message(ShmBuf *shmp, const char *msg) {
    ...
    // Mesajlar msgbuf + 4 (ID sayacından sonra)
    char *msg_start = shmp->msgbuf + sizeof(int);
    size_t used = shmp->cnt;
    size_t available = BUF_SIZE - sizeof(ShmBuf) - sizeof(int) - used - 1;
